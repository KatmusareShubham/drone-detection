...
class Model:
    ...
    def __init__(self, classes=None, device=None):
        ...
        **Example**::
        ...
        self._device = torch.device('cuda') #device if device else config['default_device']

        # Load a model pre-trained on COCO
        self._model = torchvision.models.detection.fasterrcnn_resnet50_fpn(pretrained=False)#, pretrained_backbone=False)
#         print(self._model)
    ...
    def fit(self, dataset, val_dataset, epochs=10, learning_rate=0.005, momentum=0.9,
            weight_decay=0.0005, gamma=0.1, lr_step_size=3, verbose=False, patience=5):
        ...
        **Example**::
        ...
        optimizer = torch.optim.SGD(parameters, lr=learning_rate, momentum=momentum, weight_decay=weight_decay)# changed to Adam (momentum=momentum,)
...
#                 train_losses.append(total_loss.item())
            ...
            writer.add_scalar('loss x epoch', train_losses/len(dataset), epoch)
            #print('2')
            # Validation step
            ...
            if val_dataset is not None:
                ...
                with torch.no_grad():
                    ...
                    for images, targets in val_dataset:
                        ...
                        # valid_losses.append(loss_dict.item())

                avg_loss /= len(val_dataset.dataset) #valid losses. ### hard code to 0 due to no  val data 
                train_losses/= len(dataset)
                losses.append(avg_loss)
                #print('9')
                writer.add_scalar('loss x epoch', avg_loss, epoch) #valid loss
...
#                 print('10')
                ...
                if verbose:
                    ...
                    # early stopping
                    # early_stopping(avg_loss, self._model)
        ...
        #date_time = now.strftime("%d/%m/%Y_")
        #chk= 'checkpoint_test10.pt'
        #self._model.load_state_dict(torch.load(chk))
        #writer.close()
